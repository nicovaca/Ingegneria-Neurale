function imdsTrain_mappe4D = transform4D(imdsTrain,dirname)

%Creo il dataset  costituito dagli array 4D a partire dalle mappe 3D
...
for i = 1:numFiles
    ...
    matrix3D = readFcn_N(files{1,1});
        ...
        save([percorso_images num2str(i) 'CWT' num2str(j)],'b');
...
imdsTrain_mappe4D=imageDatastore(digitDatasetPath, 'IncludeSubfolders',true,'ReadFcn', @b, 'LabelSource','foldernames',"FileExtensions",".mat");
